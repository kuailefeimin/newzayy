...
//获取二级分类name
function get_cate_two($id){
    $cate1 = M('Category')->where(array('id'=>$id))->find();
    if($cate1['pid'] == 0){
        return $cate1['title']; 
    }else{
        $cate2 = M('Category')->where(array('id'=>$cate1['pid']))->field('title')->find();
        //$url = '<a href='."{:U('Index/topic/cate_name/".$cate2['name']."')}";
        
        $data ='<a href="javascript:history.go(-1)">'.$cate2['title'].'</a>>'.$cate1['title'];
        return $data;
    }
   
}
//获取一级分类
function get_cate_one($id){
    $cate1 = M('Category')->where(array('id'=>$id))->find();
    return $cate1['title'];
}
/**
